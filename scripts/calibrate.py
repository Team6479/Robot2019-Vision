...
parser.add_argument(
    "--fisheye",
    help="Indicates that the camera has a fisheye lens",
    action="store_true",
)
parser.add_argument(
    "--fast",
    help="Continuously take pictures every second without waiting for the spacebar to be pressed",
    action="store_true",
)
parser.add_argument(
    "--num_images",
    help="The number of calibration images to take",
    action="store",
    default=13,
    type=int,
)
...
while len(imgpoints) < args.num_images:
    ...
    if args.fast or " " == chr(key & 255):
        ...
        if ret:
            ...
            num_left -= 1
            logger.info(
                "Stored a point. {} of {} left".format(num_left, args.num_images)
            )
            if args.fast:
                cv2.waitKey(1000)
...
if args.fisheye:
    ...
    ret, mtx, dist, rvecs, tvecs = cv2.fisheye.calibrate(
        fisheye_objp, fisheye_imjp, gray.shape[::-1], None, None
    )
else:
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(
        objpoints, imgpoints, gray.shape[::-1], None, None
    )
pipeline.save_calibration_results(
    mtx, dist, rvecs, tvecs, args.fisheye, constants.CALIBRATION_FILE_LOCATION
)
