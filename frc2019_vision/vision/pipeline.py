...
class BallPipeline:
    ...
    def ball_val(frame):
        ...
        frame, dist, angle = detect_ball(frame, cnts)
...
class TapePipeline:
    ...
    def get_contours(self, bitmask: np.array) -> List[np.array]:
        ...
        candidates = [approximate_contour(contour) for i, contour in enumerate(contours) if is_candidate[i]]
    ...
    def estimate_pose(self, corners_subpixel: np.array) -> Tuple[np.array, np.array, float]:
